...
class SinablogPipeline(object):
    ...
    def _do_upinsert(self, conn, item):
        ...
        conn.execute("select * from sinablog where uuid = %s", (linkmd5id, ))
        result = conn.fetchone()
        if not result:
            conn.execute("""
                insert into sinablog(uuid,title,url,authorid,published_time) 
                values(%s, %s, %s, %s, %s)
                """, (linkmd5id, item['title'], item['url'], item['authorid'], item['date']))
