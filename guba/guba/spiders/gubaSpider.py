...
class GubaSpider(scrapy.Spider):
    ...
    base_url = "http://guba.sina.com.cn/?s=bar&name=%C3%FB%BC%D2%C2%DB%CA%D0&type=0&page="
    start_urls = [base_url+str(i) for i in range(1,2575)]
    ...
    def parse(self, response):
        ...
        for i in range(len(h)/5):
            ...
            if h[5*i+3].div.a:
                item["weiboid"] = mode.findall(h[5*i+3].div.a["href"])[0]
            else:
                item["weiboid"] = None
